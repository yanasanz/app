...
class PostDaoImpl(private val db: SQLiteDatabase) : PostDao {
    ...
    companion object {
        ...
        val DDL = """
            ...
            ${PostColumns.COLUMN_LIKES_AMOUNT} INTEGER DEFAULT 0,
            ${PostColumns.COLUMN_SHARED_BY_ME} BOOLEAN NOT NULL DEFAULT 0,
            ${PostColumns.COLUMN_SHARES_AMOUNT} INTEGER DEFAULT 0
            );
            """.trimIndent()
        val DELETE = "DROP TABLE IF EXISTS ${PostColumns.TABLE}"
    }
    ...
    object PostColumns {
        ...
        const val COLUMN_SHARES_AMOUNT = "sharesAmount"
    ...
    private fun map(cursor: Cursor): Post {
        with(cursor) {
            return Post(
                ...
                sharesAmount = getInt(getColumnIndexOrThrow(PostColumns.COLUMN_SHARES_AMOUNT))
            )
        }
    }
}